# Better to use Raw Implementation from deepspeed
...
class TopKGate(Module):
    ...
    def forward(self,
                input: torch.Tensor) -> Tuple[Tensor, Tensor, Tensor]:  # type: ignore
...
class MOELayer(Base):
    ...
    def __init__(self,
        ...
        if num_tokens:
            self.vision_embed = nn.Parameter(torch.zeros(self.gate.model_dim))
            trunc_normal_(self.vision_embed, std=0.02)
            self.text_embed = nn.Parameter(torch.zeros(self.gate.model_dim))
            trunc_normal_(self.text_embed, std=0.02)
            self.num_vision_tokens = num_tokens

    def forward(self, *input: Tensor, mode='vl', **kwargs: Any) -> Tensor:
        ...
        if self.num_vision_tokens:
            if mode == 'vision':
                input_gate = input[0].clone() + self.vision_embed
            elif mode == 'text':
                input_gate = input[0].clone() + self.text_embed
            else:
                input_gate = input[0].clone()
                input_gate[:, :self.num_vision_tokens] = input_gate[:, :self.num_vision_tokens] + self.vision_embed
                input_gate[:, self.num_vision_tokens:] = input_gate[:, self.num_vision_tokens:] + self.text_embed
            reshaped_input = input_gate.reshape(-1, d_model)
        else:
