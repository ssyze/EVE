...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cross_emb, cross_mask=None):
        B, N, C = x.shape
        B, N_c, C = cross_emb.shape
        q = self.q_linear(x).reshape(B, -1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        kv = self.kv_linear(cross_emb).reshape(B, N_c, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]   # make torchscript happy (cannot use tensor as tuple)
        # q: (bs, len_q, dim), k: (bs, len_k, dim) q @ k: (bs, len_q, len_k)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        if cross_mask is not None:
            cross_mask = cross_mask.bool()
            attn = attn.masked_fill(~cross_mask[:, None, None, :], float("-inf"))
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
...
class Block(nn.Module):
    ...
    def forward(self, x, cross_emb=None, cross_atts=None, register_hook=False, image_atts=None):
        x = x + self.drop_path(self.attn(self.norm1(x), register_hook=register_hook, image_atts=image_atts))
        if cross_emb is not None:
            x = x + self.drop_path(self.cross_attn(self.norm_q(x), self.norm_kv(cross_emb), cross_atts))
