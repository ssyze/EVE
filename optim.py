...
import fairscale.optim
import torch.optim


def create_optimizer(args, model, fs=False):
    lr = args.lr
    ...
    if fs:
        optimizer = fairscale.optim.OSS(optimizer_grouped_parameters, optim=AdamW, lr=lr, eps=1e-8, betas=(0.9, 0.98))
    else:
